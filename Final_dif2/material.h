...
public:
    swirl(const color& a, double frequency, double time_factor)
        : albedo(a), freq(frequency), time_factor(time_factor) {}
    ...
    virtual bool scatter(
        ...
        vec3 p = freq * rec.p;
        double scale = sin(p.x() * time_factor) * sin(p.y() * time_factor) * sin(p.z() * time_factor);
